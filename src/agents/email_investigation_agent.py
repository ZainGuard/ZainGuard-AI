...
class EmailInvestigationAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.phishing_indicators = [
            ...
            "click here", "limited time", "act now", "expires soon",
    ...
    async def process_task(self, task: AgentTask) -> Dict[str, Any]:
        ...
        try:
            ...
            if "headers" in email_data:
                results["header_analysis"] = await self._analyze_email_headers(email_data["headers"])
            ...
            urls = await self._extract_urls(email_data.get("content", email_data.get("body", "")))
            if urls:
                results["url_analysis"] = await self._analyze_urls(urls)
            
            # 4. Analyze attachments
            if "attachments" in email_data:
                results["attachment_analysis"] = await self._analyze_attachments(email_data["attachments"])
            ...
            results["phishing_analysis"] = await self._detect_phishing_patterns(email_data)
    ...
    async def _analyze_email_headers(self, headers: Dict[str, str]) -> Dict[str, Any]:
        ...
        if "from" in headers and "reply-to" in headers:
            if headers["from"] != headers["reply-to"]:
                analysis["spoofing_indicators"].append("From and Reply-To addresses differ")
        ...
        if "authentication-results" in headers:
            analysis["authentication_results"]["dkim"] = headers["authentication-results"]
        ...
        if "received" in headers:
            ...
            for received in received_headers:
                if any(indicator in received.lower() for indicator in ["relay", "proxy", "anonymizer"]):
        ...
        for url in urls:
            try:
                ...
                if domain not in analysis["domain_analysis"]:
                    try:
                        domain_reputation = await self.execute_tool("check_domain_reputation", domain=domain)
                        analysis["domain_analysis"][domain] = domain_reputation
                    except Exception as e:
                        logger.warning(f"Failed to check domain reputation for {domain}: {e}")
    ...
    async def _analyze_attachments(self, attachments: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for attachment in attachments:
            ...
            if file_hash:
                try:
                    hash_analysis = await self.execute_tool("check_file_hash", file_hash=file_hash)
    ...
    async def _detect_phishing_patterns(self, email_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        urgency_words = ["urgent", "immediate", "asap", "expires", "limited time", "act now"]
        ...
        suspicious_requests = ["verify", "confirm", "update", "click here", "login", "password"]
        request_count = sum(1 for word in suspicious_requests if word in content.lower())
        if request_count > 0:
            phishing_score += 15 * request_count
            indicators.append(f"Suspicious requests detected ({request_count} indicators)")
        
        # Check subject line
        if any(word in subject.lower() for word in ["urgent", "verify", "suspended", "alert"]):
            phishing_score += 25
    ...
    async def _generate_risk_assessment(self, investigation_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _generate_recommendations(self, investigation_results: Dict[str, Any]) -> List[str]:
        ...
        if investigation_results.get("attachment_analysis", {}).get("suspicious_attachments"):
        ...
        if investigation_results.get("header_analysis", {}).get("spoofing_indicators"):
            recommendations.append("Implement stricter email authentication (DMARC, DKIM, SPF)")
