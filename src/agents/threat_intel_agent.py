...
class ThreatIntelAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.register_tool(
            "search_threat_actors", self.threat_intel.search_threat_actors
        )
        self.register_tool(
            "get_malware_families", self.threat_intel.get_malware_families
        )
    ...
    async def _analyze_ioc(self, ioc_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._store_threat_intelligence(
                ioc_type, ioc_value, threat_data, analysis
            )
    ...
    async def _gather_ioc_intelligence(
        ...
        try:
            ...
            logger.warning(
                f"Error gathering intelligence for {ioc_type}:{ioc_value}: {e}"
            )

        return threat_data

    async def _analyze_threat_data(
        self, ioc_type: str, ioc_value: str, threat_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Analyze threat data using AI."""
        try:
            # Create analysis prompt
            prompt = self._create_threat_analysis_prompt(
                ioc_type, ioc_value, threat_data
            )
...
""",
    ...
    async def _bulk_ioc_analysis(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for ioc in iocs:
            try:
                ...
                results.append({"ioc": ioc, "status": "success", "result": result})
            except Exception as e:
                results.append({"ioc": ioc, "status": "error", "error": str(e)})
    ...
    async def _analyze_hunt_findings(
        self, findings: List[Dict[str, Any]], criteria: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_hunt_recommendations(
        self, findings: List[Dict[str, Any]], risk_level: str
    ) -> List[str]:
        ...
        if len(findings) > 10:
            recommendations.append(
                "High volume of findings - consider automated filtering"
            )
        ...
        if "apt" in categories:
            recommendations.append(
                "APT activity detected - escalate to senior analysts"
            )
    ...
    async def _generate_ai_report(
        self, threat_data: Dict[str, Any], report_type: str
    ) -> Dict[str, Any]:
