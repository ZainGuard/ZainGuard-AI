...
class TriageAgent(BaseAgent):
    ...
    async def _analyze_alert(self, alert_details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            priority_score = self._calculate_priority_score(
                alert_details, analysis, threat_data
            )
            ...
            return {
                ...
                "false_positive_likelihood": analysis.get(
    ...
    def _prepare_alert_context(self, alert_details: Dict[str, Any]) -> str:
        ...
        if alert_details.get("raw_data"):
            context_parts.append(
                f"Raw Data: {json.dumps(alert_details['raw_data'], indent=2)}"
            )

        return "\n".join(context_parts)

    async def _gather_threat_intelligence(
        self, alert_details: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for ip in iocs.get("ips", []):
                threat_data[f"ip_{ip}"] = await self.threat_intel.check_ip_reputation(
                    ip
                )

            # Check domains
            for domain in iocs.get("domains", []):
                threat_data[f"domain_{domain}"] = (
                    await self.threat_intel.check_domain_reputation(domain)
                )

            # Check file hashes
            for file_hash in iocs.get("hashes", []):
                threat_data[f"hash_{file_hash}"] = (
                    await self.threat_intel.check_file_hash(file_hash)
                )
    ...
    def _rule_based_analysis(
        self, alert_details: Dict[str, Any], threat_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _calculate_priority_score(
        ...
        malicious_count = sum(
            1
            for data in threat_data.values()
            if isinstance(data, dict) and data.get("malicious", False)
        )
    ...
    async def _batch_triage(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for alert_id in alert_ids:
            try:
                ...
                results.append(
                    {"alert_id": alert_id, "status": "error", "message": str(e)}
                )
