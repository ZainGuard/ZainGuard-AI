...
class BaseAgent(ABC):
    ...
    async def _wait_for_task(self, task_id: str):
        ...
        while task_id in self.current_tasks and self.current_tasks[task_id].status in [
            "pending",
            "running",
        ]:
    ...
    async def submit_task(
        self, task_type: str, input_data: Dict[str, Any], priority: int = 1
    ) -> str:
...
class AgentManager:
    ...
    def create_agent(
        self, agent_type: str, agent_id: str, name: str, description: str, **kwargs
    ) -> BaseAgent:
