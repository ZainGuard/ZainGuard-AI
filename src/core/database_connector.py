...
class ChromaVectorStore(VectorStore):
    ...
    def __init__(
        self, collection_name: str = "zain_guard_kb", persist_directory: str = None
    ):
        ...
        if self._connection is None:
            if self.database_url.startswith("sqlite"):
                ...
                raise NotImplementedError(
                    f"Database type not supported: {self.database_url}"
                )
                ...
                events.append(
                    {
                        ...
                        "source": row["source"],
                        "raw_data": (
                            json.loads(row["raw_data"]) if row["raw_data"] else {}
                        ),
                        "processed_data": (
                            json.loads(row["processed_data"])
