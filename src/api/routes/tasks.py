...
async def submit_task(agent_id: str, request: SubmitTaskRequest):
    ...
    try:
        ...
        return {"task_id": task_id, "agent_id": agent_id, "status": "submitted"}
...
async def submit_batch_tasks(agent_id: str, tasks: List[SubmitTaskRequest]):
    ...
    try:
        ...
        for task_request in tasks:
            try:
                ...
                results.append(
                    {"task_id": task_id, "status": "submitted", "success": True}
                )
            except Exception as e:
                results.append(
                    {
