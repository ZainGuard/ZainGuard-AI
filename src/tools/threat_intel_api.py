...
class ThreatIntelAPI:
            ...
            if vt_result:
                ...
                if vt_result.get("malicious", False):
                    ...
                    result["confidence"] = max(
                        result["confidence"], vt_result.get("confidence", 0)
                    )
        ...
        if self.abuseipdb_api_key:
            ...
            if abuse_result:
                ...
                if abuse_result.get("malicious", False):
                    result["malicious"] = True
                    result["confidence"] = max(
                        result["confidence"], abuse_result.get("confidence", 0)
                    )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    "confidence": (
                        malicious_count / total_engines if total_engines > 0 else 0
                    ),
    ...
    async def _check_virustotal_domain(self, domain: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            headers = {"x-apikey": self.virustotal_api_key}
            response = await client.get(
                f"https://www.virustotal.com/api/v3/domains/{domain}", headers=headers
            )
