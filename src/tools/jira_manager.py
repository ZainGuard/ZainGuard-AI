    ...
    async def _get_client(self) -> httpx.AsyncClient:
        ...
        if self._client is None:
            ...
            self._client = httpx.AsyncClient(
                base_url=self.base_url, auth=auth, headers=headers, timeout=30.0
            )
        return self._client
    ...
    async def update_ticket(self, ticket_key: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            response = await client.put(
                f"/rest/api/3/issue/{ticket_key}", json=jira_updates
            )
            response.raise_for_status()
    ...
    async def add_comment(self, ticket_key: str, comment: str) -> bool:
        ...
        try:
            ...
            response = await client.post(
                f"/rest/api/3/issue/{ticket_key}/comment", json=comment_data
            )
            response.raise_for_status()
    ...
    async def transition_ticket(self, ticket_key: str, transition_name: str) -> bool:
        ...
        try:
            ...
            if not transition_id:
                logger.warning(
                    f"Transition '{transition_name}' not found for ticket {ticket_key}"
                )
                return False
            ...
            response = await client.post(
                f"/rest/api/3/issue/{ticket_key}/transitions", json=transition_data
            )
            response.raise_for_status()
    ...
    async def search_tickets(
        ...
        try:
            ...
            search_data = {
                ...
                "fields": fields
                or ["key", "summary", "status", "priority", "assignee", "created"],
            }
